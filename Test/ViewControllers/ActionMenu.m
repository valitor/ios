...
- (IBAction)scanOnPressed:(id)sender {
    ...
    //Start the scanner and configure it for all barcodes
    [[CommunicationManager manager] startScan];
    
    //See different methods in the communication manager on how to configure the scanner
    //For different types of barcodes
    //[[CommunicationManager manager] configureScannerForQRorAztec];
    [[CommunicationManager manager] configureScannerForAllSymbols];
}
...
-(void)setupUIBasedOnPickerResponse{
    ...
    switch (_pickerResponse) {
        ...
        case PickerResponseAuthorization:
            ...
            break;
        case PickerResponseAuthorizationWithPing:
            _txtAmount.hidden = NO;
            break;
...
-(void)sendAuthWithPing{
    
    //An example on how you can check for connectivity with the POS device
    //Using the PING message
    //The PING message only has 5 seconds for timeout unlike the other requests
    //So if you get a response from the POS device
    //You know that you have a good connection with the POS device and
    //can send, an authorization for example, in the completion block of the PING request
    ...
    [[CommunicationManager manager] demandPingWithCompletionBlock:^(BOOL success, NSString *strRawResponse, NSString *strMsgDelivered) {
        ...
        if(success){
            [manager.arrConsoleMsgs addObject:@"Successful, sending AUTH"];
            
            
            [self sendAuthorizationWithAmount:[_txtAmount.text intValue]];
        }
        else{
            [manager.arrConsoleMsgs addObject:@"Not Successful, not sending AUTH"];
        }
